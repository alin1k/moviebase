...
const discoverByGenresURL = (genres, page) =>
    `https://api.themoviedb.org/3/discover/movie?api_key=${process.env.TMDB_API_KEY}&sort_by=popularity.desc&with_genres=${genres}&page=${page}&include_adult=false`
...
export default async function handler(req, res) {
    ...
    switch (method) {
        ...
        default:
            res.status(404).json({message: `${method} method not supported on path /api/discover-genres`});
            break;
    }
}